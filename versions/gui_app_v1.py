...
class App:
    ...
    def _build_sidebar(self):
        ...
        # Logo area
        logo_frame = tk.Frame(inner, bg=C["sidebar"])
        logo_frame.pack(fill="x", pady=(0, 20))

        # Draw the ArgoCD-style logo
        logo_cv = tk.Canvas(logo_frame, width=48, height=48,
                            bg=C["sidebar"], highlightthickness=0)
        logo_cv.pack(anchor="w")
        # Circle
        logo_cv.create_oval(4, 4, 44, 44, outline=C["accent"], width=2)
        # Inner sync arrows
        logo_cv.create_arc(12, 12, 36, 36, start=30, extent=120,
                           outline=C["green"], width=2, style="arc")
        logo_cv.create_arc(12, 12, 36, 36, start=210, extent=120,
                           outline=C["accent"], width=2, style="arc")
        # Center dot
        logo_cv.create_oval(21, 21, 27, 27, fill=C["accent"], outline=C["accent"])

        tk.Label(inner, text="ArgoCD", bg=C["sidebar"],
                 fg=C["fg_bright"], font=("Segoe UI", 14, "bold")).pack(anchor="w")
        tk.Label(inner, text="Log Analyzer", bg=C["sidebar"],
                 fg=C["accent"], font=("Segoe UI", 12)).pack(anchor="w")

        # Divider
        tk.Frame(inner, height=1, bg=C["border"]).pack(fill="x", pady=16)

        # Quick commands
        tk.Label(inner, text="QUICK ACTIONS", bg=C["sidebar"],
                 fg=C["fg_dim"], font=FONT_XS).pack(anchor="w", pady=(0, 8))

        cmds = [
            ("\U0001f4c2  Open File",     self._browse),
            ("\U0001f4cb  Paste Logs",    self._focus_input),
            ("\U0001f50d  Analyze",       self._start),
            ("\U0001f4be  Save Report",   self._save),
            ("\U0001f5d1  Clear All",     self._clear_all),
        ]
        for label, cmd in cmds:
            b = tk.Label(inner, text=label, bg=C["sidebar"], fg=C["fg"],
                         font=FONT, cursor="hand2", anchor="w", padx=8, pady=4)
            b.pack(fill="x", pady=1)
            b.bind("<Enter>", lambda e, w=b: w.configure(bg=C["border"], fg=C["fg_bright"]))
            b.bind("<Leave>", lambda e, w=b: w.configure(bg=C["sidebar"], fg=C["fg"]))
        ...
        # Divider
        tk.Frame(inner, height=1, bg=C["border"]).pack(fill="x", pady=10)

        # Footer info
        tk.Label(inner, text="Keyboard Shortcuts", bg=C["sidebar"],
                 fg=C["fg_dim"], font=FONT_XS).pack(anchor="w", pady=(0, 4))
        for shortcut in ["Ctrl+O    Open file", "Ctrl+Enter Analyze",
                         "Ctrl+S    Save report"]:
            tk.Label(inner, text=shortcut, bg=C["sidebar"],
                     fg=C["fg_dim"], font=("Cascadia Code", 8)).pack(anchor="w")

        tk.Frame(inner, height=1, bg=C["border"]).pack(fill="x", pady=(10, 6))
        tk.Label(inner, text="Powered by Bosch LLM", bg=C["sidebar"],
                 fg=C["fg_dim"], font=FONT_XS).pack(anchor="w")
        tk.Label(inner, text="v1.0.0", bg=C["sidebar"],
                 fg=C["border"], font=FONT_XS).pack(anchor="w")
